...
$query2 = "SELECT COUNT(*) FROM `products` WHERE `subcategory`='".mysqli_real_escape_string($conn, $search)."' AND `availability`='1' AND `buyer`='0'";
...
while($loopscount <= $pagecount+1){
        ...
        if($loopscount == $page){
            echo "<li class=\"active\"><a href=\"mainpageesub.php?search=$search&page=$loopscount\">$loopscount</a></li>";
        }else{
            echo "<li><a href=\"mainpageesub.php?search=$search&page=$loopscount\">$loopscount</a></li>";
        }
    $loopscount++;
