...
if(isset($_POST['current'])){
    ...
    <div class=\"col-lg-12 col-md-12 col-sm-12 col-xs-12\">";
        echo "<h4 class=\"text-center text-capitalize\">Currently showing blogs published by Nick</h4>";
...
if($current == 1){
    ...
    if($current > 1){
        ...
        if($current == $pages){
            ...
            if($current < $pages){
            echo "<a onclick=\"bloggeronlineitems($current - 1);\" class=\"btn btn-warning btn-xs\">Prev</a>";
                echo "Showing page $current of $pages pages";
            echo "<a onclick=\"bloggeronlineitems($current + 1);\" class=\"btn btn-warning btn-xs\">Next</a>";
            }
        }
    }
}
...
while($Query_fetch_assoc = mysqli_fetch_assoc($query_run)){
   ...
   // echo $Query_fetch_assoc['id'].'<br>';
    ...
    echo "<td>";
    echo "<a onclick=\"previewblog($blogid);\" class=\"btn btn-info btn-xs\">Preview</a>";
    echo "</td>";
