...
$category = 'misc';
$subcategory = 'mensmisc';
$notifymecount = 4;// 400 100 in order
$today = Date('Y-m-d');
$toolowcustomer = 1;
$toolowmessagee = 'items in '.$category.' CATEGORY, in '.$subcategory.' need restocking';
$query = "SELECT * FROM `products` WHERE `category`='$category' AND `sex`!='female' AND `availability`='1' AND `buyer`='0'";
...
$query2 = "SELECT * FROM `products` WHERE `category`='$category' AND `sex`!='female' AND `availability`='1' AND `buyer`='0' ORDER BY RAND() LIMIT 6";
