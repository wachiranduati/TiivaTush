...
$query2 = "SELECT COUNT(*) FROM `products` WHERE `category`!='interiors' AND `category`!='kids' AND `sex`!='".mysqli_real_escape_string($conn, $search)."' AND `availability`='1' AND `buyer`='0'";
...
<!doctype html>
<html>
    <title>Tiiva.co.ke | Mtumba | <?php echo $gender;?></title>
    <head>
...
<div class="container-fluid">
            ...
            <div class="row visible-sm visible-xs" style="margin-top:5px;">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="border-bottom:2px solid orange;">
                    <div class="row">
                        <div class="col-lg-2 col-md-2 col-sm-12 col-xs-12" style="border-bottom:1px solid white;background-color:black;margin-bottom:5px;"><h4 class="text-center text-uppercase" style="font-weight:bold;color:white;"><a href="item.html" style="color:white;" class="btn-block"><?php echo $gender;?></a></h4></div>
                    </div>
...
while($loopscount <= $pagecount+1){
        ...
        if($loopscount == $page){
            echo "<li class=\"active\"><a href=\"mainpageesex.php?search=$search&page=$loopscount\">$loopscount</a></li>";
        }else{
            echo "<li><a href=\"mainpageesex.php?search=$search&page=$loopscount\">$loopscount</a></li>";
        }
    $loopscount++;
