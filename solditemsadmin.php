...
if(isset($_GET['sales'])){
  ...
  $query_run = mysql_query($query);
          ...
          $querycount_run = mysql_query($querycount);
          $count =  mysql_num_rows($querycount_run);
          $pagecount = $count - 1;
          $pages = ceil($count/10);
          //echo $pages;
          if($button == 1 ){
              if($pages == 1){
                  echo " Showing page $button of $pages ";
                  //echo "<a class=\"btn btn-primary btn-sm\" onclick=\"sales($button + 1);\">Next</a>";
              }else{
                  echo " Showing page $button of $pages ";
                  echo "<a class=\"btn btn-primary btn-sm\" onclick=\"sales($button + 1);\">Next</a>";
              }
              ...
              if($button > 1){
                   $newbutton = $pages - 1;
                  if($button < $pages){
                      echo "<a class=\"btn btn-primary btn-sm\" onclick=\"sales($button - 1);\">Previous</a>";
                      echo " Showing page $button of $pages";
                      echo "<a class=\"btn btn-primary btn-sm\" onclick=\"sales($button + 1);\">Next</a>";
                  }else{
                      if($button == $pages){
                      echo "<a class=\"btn btn-primary btn-sm\" onclick=\"sales($button - 1);\">Previous</a> Showing last page";
                      }
  ...
  $count = ceil(1 * $jump) + 1;
  echo "<h4 class=\"text-center text-uppercase\">New Sales <small class=\"text-capitalize\" style=\"color:white;text-shadow:0 1px 1px black;\">Yet to be pickedup!!</small></h4>";
  echo "
