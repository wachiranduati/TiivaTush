...
if($page == 1){
    ...
    if($dbs == 'products'){
        ...
        $query = "SELECT * FROM $dbs WHERE `instock`>='1' AND `itemtitle` LIKE '%".mysql_real_escape_string($search)."%' LIMIT $items1";
    }else{
        $query = "SELECT * FROM $dbs WHERE `instock`>='1' AND `itemtitle` LIKE '%".mysql_real_escape_string($search)."%' LIMIT $items OFFSET $jump";
    }
}else{
...
if($dbs == 'products'){
    $query2 = "SELECT * FROM $dbs WHERE `availability`='1' AND `buyer`='0' AND `itemtitle` LIKE '%".mysql_real_escape_string($search)."%' LIMIT $items OFFSET $jump";
}elseif( $dbs == 'brandyproducts'){
    $query2 = "SELECT * FROM $dbs WHERE `instock`>='1' AND `itemtitle` LIKE '%".mysql_real_escape_string($search)."%' LIMIT $items OFFSET $jump";
}else{
    $query2 = "SELECT * FROM `brandyproducts` WHERE `instock`>='1' AND `itemtitle` LIKE '%".mysql_real_escape_string($search)."%' LIMIT $items OFFSET $jump";
}
...
$price = number_format($query_row['price']);
    ...
    <div class=\"col-lg-2 col-md-2 col-sm-6 col-xs-6\" id=\"box1\" style=\"margin-top:1%;border:none;\">
            <a href=\"productdetails.php?id=$query_row[id]\" target=\"_blank\">
                <div class=\"col-lg-12 col-md-12 col-sm-12 col-xs-12 productimage\" style=\"background-image:url('$imageaddr$query_row[imageone]'); height:140px;width:100%;background-size:80%;\">
